...
try:
    ...
    print(
        "settings_secret.py is deprecated. Please migrate to using settings_local.py. "
...
DATABASES = {
    "default": env.db(
        "QLEVERUI_DATABASE_URL",
        default=f"sqlite:////{os.path.join(BASE_DIR, 'db', 'qleverui.sqlite3')}",
    )
}
...
try:
    ...
    STATIC_VERSION = "Git commit {} on {}".format(GIT_HASH, GIT_HEAD.split("/")[-1])
...
if not STATIC_VERSION:
    ...
    try:
        versionInfo = (subprocess.check_output("svn info -r HEAD;", shell=True)).decode(
            "utf-8"
        )
