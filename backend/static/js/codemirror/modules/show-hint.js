...
(function(mod) {
  ...
  function Widget(completion, data) {
	    ...
	    if (percentage >= 0.7 && requestExtension == false && found-10 > lastWidget.data.list.length){
		  ...
		  fetchHints(lastWidget.completion.options.hint, lastWidget.cm, lastWidget.completion.options, function(data) {
			if(data != null) {
			    ...
			    var newData = lastWidget.data.list.concat(data.list);
				lastWidget.data.list = newData;
				
				var hints = document.getElementsByClassName('CodeMirror-hints')[0]
    ...
    var box = hints.getBoundingClientRect(), overlapY = box.bottom - winH - 100;
  ...
  Widget.prototype = {
    ...
    changeActive: function(i, avoidWrap) {
      ...
      var node = this.hints.childNodes;
      for(var k = 0; k < node.length; k++){
	      node[k].className = node[k].className.replace(" " + ACTIVE_HINT_ELEMENT_CLASS, "");
	  }
