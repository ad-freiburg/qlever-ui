...
class Command(BaseCommand):
    help = (
        "Usage: python manage.py cleanup <backends to keep>\n"
        "\n"
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "backend_slugs", nargs="*", help="Slugs of the backends to keep"
        )
    ...
    def handle(self, *args, returnLog=False, **options):
        ...
        Backend.objects.exclude(slug__in=backend_slugs).exclude(
            slug__contains="globaldefaults"
        ).delete()
        # Remove all links.
        ...
        User.objects.create_user(
            username="demo", password="demo", is_staff=True, is_superuser=True
        )
        # Remove all sessions.
