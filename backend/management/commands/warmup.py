...
class Command(BaseCommand):
    ...
    def _buildQuery(self, completionQuery):
        ...
        for (
            placeholder,
            replacement,
        ) in self.backend.getWarmupAndAcPlaceholders().items():
    ...
    def _getPrefixString(self):
        prefixString = "\n".join(
            [
                f"PREFIX {prefixName}: <{prefix}>"
                for prefixName, prefix in self.backend.availablePrefixes.items()
            ]
        )
        return prefixString
